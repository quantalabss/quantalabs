...
export const metadata: Metadata = {
  ...
  description:
    "Latest insights on post-quantum cryptography, CBOMs, and sovereign infrastructure.",
};
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12">
            ...
            <div className="lg:col-span-4 p-10 flex flex-col justify-end bg-[#C4ED5F] relative overflow-hidden border-l-2 border-black">
              ...
              <p className="text-sm text-black font-medium relative z-10">
                Analysis, engineering updates, and regulatory insights from the
                frontier of post-quantum cryptography.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1">
            {blogPosts.map((post, index) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className={`group flex flex-col md:flex-row md:items-stretch hover:bg-gray-50 transition-colors ${index !== blogPosts.length - 1 ? "border-b-2 border-black" : ""}`}
              >
                {/* Meta Column */}
                <div className="w-full md:w-64 p-8 border-b-2 md:border-b-0 md:border-r-2 border-black flex flex-row md:flex-col justify-between md:justify-start items-center md:items-start bg-gray-50/30 group-hover:bg-white transition-colors">
                  ...
                  <div className="flex flex-col gap-2 mt-0 md:mt-4">
                    {post.tags.slice(0, 2).map((tag) => (
                      <span
                        key={tag}
                        className="font-mono text-[9px] font-bold uppercase tracking-widest text-black bg-[#C4ED5F] px-2 py-1"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                {/* Action Column */}
                <div className="w-full md:w-24 p-6 border-t-2 md:border-t-0 md:border-l-2 border-black flex items-center justify-center bg-white group-hover:bg-[#C4ED5F] transition-colors">
